...
//将aedat数据转换成ts, x, y, pol
def get_davis_dvs_events(file, ROI=np.array([]), numEvents=1e10, startEvent=0):
    print('\nget_davis_dvs_events function called \n')
    sizeX = 346
    sizeY = 260
    x0 = 0
    y0 = 0
    ...
    if len(ROI) != 0:
        if len(ROI) == 4:
            ...
            print(
                'Unknown ROI argument. Call function as: \n get_davis_dvs_events(file, ROI=[x0, y0, x1, y1], numEvents=nE, startEvent=sE) '
                'to specify ROI or\n get_davis_dvs_events(file, numEvents=nE, startEvent=sE) to not specify ROI')
            return
...
//对ts, x, y, pol数据进行 邻近过滤
def denoise_events(data=np.array([[]]), height=260, width=346, margin=2, threshold=2):
...
sub_num = 11         # subject number of filtration 
...
for k in range(0, sub_num):
    for m in range(0, sub_group):
        ...
        ts, x, y, pol = get_davis_dvs_events(folder_name + data_file[k*sub_group+m])
        ...
        while ((end_time <= raw_data[-1,3]) & (end_time < end_ts)):    #raw_data[-1,3] = the last timestamp        
            ...
            if filter_flag is True:
                data_filter = denoise_events(data, height=260, width=346, margin=2, threshold=8)   
